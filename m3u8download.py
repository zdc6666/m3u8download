        ...
        for tsurl in m3u8list:
            ...
            try:  # 报错防止跳出
                if os.path.exists(f'./{name}/ts{count}.mp4'):
                    ...
                    ts = requests.get(tsurl, stream=True, headers=headers, allow_redirects=True, timeout=10)#核心代码，下载每一个ts文件
                    ts.encoding = ts.apparent_encoding
                    print(f'视频{dir}，总共有{len(m3u8list)}个ts片段，第{str(count)}个已下载完成，key为{keykey},状态为{ts.status_code}')
                ...
                print(f'视频{name}，总共有{len(m3u8list)}个ts片段，第{str(count)}个下载失败(可能断网)，状态为{ts.status_code}')
                #tsurl count
                continue
